...
- **1 epoch** ≈ 1 day (varies by network)
  ...
  if (error) {
    return (
      <div className="cost-display error">
        <p>⚠️ Unable to calculate costs. Please try again.</p>
      </div>
    );
  }
    ...
    <div className="cost-display">
      ...
      <div className="cost-summary">
        ...
        <p className="disclaimer">
          💡 Costs are estimates and may vary based on current network conditions.
        </p>
      </div>
    </div>
  );
...
const processAssets = (assets: IAsset[]): IAsset[] => {
  return assets.map(asset => {
    ...
    if (['html', 'css', 'js'].includes(fileExt || '')) {
      ...
      console.log(`${asset.path}: ${asset.content.byteLength} → ${compressedContent.byteLength} bytes`);
