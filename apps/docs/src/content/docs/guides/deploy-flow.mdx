...
const handleUploadAssets = async () => {
  try {
    ...
    console.log('✅ Assets uploaded for 5 epochs');
  } catch (error) {
...
const handleUploadAssets = async () => {
  try {
    ...
    console.log('✅ Assets uploaded permanently');
  } catch (error) {
    console.error('❌ Upload failed:', error);
  }
};
...
const handleCertifyAssets = async () => {
  try {
    ...
    console.log('✅ Assets certified successfully');
    ...
    console.error('❌ Asset certification failed:', error);
  }
};
```
  ...
  return (
    <div className="deploy-section">
      ...
      <div className="deploy-buttons">
        ...
        >
          {currentStep <= DeploySteps.Idle ? '1. Register Blobs' : '✓ Registered'}
        </button>
        ...
        >
          {currentStep <= DeploySteps.Prepared ? '2. Upload to Network' : '✓ Uploaded'}
        </button>
        ...
        >
          {currentStep <= DeploySteps.Uploaded ? '3. Certify Upload' : '✓ Certified'}
        </button>
        ...
        >
          {currentStep <= DeploySteps.Certified ? '4. Update Site' : '✓ Updated'}
        </button>
      </div>
