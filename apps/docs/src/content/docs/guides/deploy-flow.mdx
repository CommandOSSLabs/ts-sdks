...
The deployment flow consists of 5 sequential steps:

<Steps>

1. **Prepare Resources** - Validate and prepare your files for deployment
2. **Encode Resources** - Encode files into blobs and compute blob IDs
3. **Write Resources** - Register and upload blobs to the Walrus decentralized storage network
4. **Certify Resources** - Create on-chain certificates for uploaded blobs on Sui
5. **Write Site** - Create or update your site with the certified resources
...
- ✅ **Wallet Connected** - Sui wallet with sufficient funds for gas fees
- ✅ **Files Prepared** - Your website files ready for deployment
- ✅ **Network Access** - Connection to Walrus and Sui networks
- ✅ **Gas Budget** - Sui tokens for blockchain transactions
...
The first step validates and prepares your files for deployment:

```typescript
const handlePrepareResources = async () => {
  try {
    console.log('🚀 Starting resource preparation...');
    ...
    console.error('❌ Resource preparation failed:', error);
  }
};
```
...
- Site diff is computed to determine what needs to be uploaded

### Step 2: Encode Resources

Encode your prepared files into blobs and compute their blob IDs:

```typescript
const handleEncodeResources = async () => {
  try {
    console.log('🔧 Encoding resources into blobs...');
    
    await deployFlow.encodeResources();
    console.log('✅ Resources encoded successfully');
  } catch (error) {
    console.error('❌ Resource encoding failed:', error);
  }
};
```

**What happens during encoding:**
- Files are encoded into Walrus blob format
- Blob IDs are computed using content hashing
- Blobs are prepared for upload to the network
...
const handleWriteResources = async () => {
  try {
    console.log('📤 Uploading resources for extended storage...');
    
    // Write for 5 epochs (approximately 5 days on testnet)
    await deployFlow.writeResources(5, false);
    
    console.log('✅ Resources uploaded for 5 epochs');
  } catch (error) {
...
const handleWriteResources = async () => {
  try {
    console.log('📤 Uploading resources for permanent storage...');
    
    // Write with maximum epochs and permanent flag
    await deployFlow.writeResources('max', true);
    
    console.log('✅ Resources uploaded permanently');
  } catch (error) {
    console.error('❌ Upload failed:', error);
  }
};
...
const handleCertifyResources = async () => {
  try {
    console.log('🔐 Starting resource certification...');
    
    const { certifiedBlobs } = await deployFlow.certifyResources();
    
    console.log('✅ Resources certified successfully');
    ...
    console.error('❌ Resource certification failed:', error);
  }
};
```

**Certification provides:**
- 🔐 **On-chain proof** of file existence and integrity
- 🔗 **Immutable references** that cannot be tampered with
- 🌐 **Sui integration** for decentralized verification
- 📝 **Audit trail** of all deployment transactions
...
const handleWriteSite = async () => {
  try {
    console.log('🌐 Creating/updating site...');
...
import { WalrusSiteBuilderSdk, type ICertifiedBlob } from '@cmdoss/site-builder';
import { ZenFsFileManager } from '@cmdoss/file-manager';
import { useSuiClient, useCurrentAccount, useSignAndExecuteTransaction } from '@mysten/dapp-kit';
import { WalrusClient } from '@mysten/walrus';
...
function DeploymentComponent() {
  ...
  const handleEncodeResources = async () => {
    ...
    try {
      await deployFlow?.encodeResources();
      setCurrentStep(DeploySteps.Encoded);
    } catch (error) {
  ...
  return (
    <div className="deployment-flow">
      ...
      <div className="deploy-steps">
        ...
        >
          {currentStep > DeploySteps.Idle ? '✅ Prepared' : '1. Prepare Resources'}
        </button>

        <button 
          onClick={handleEncodeResources}
          disabled={currentStep !== DeploySteps.Prepared || loading}
          className={currentStep > DeploySteps.Prepared ? 'completed' : ''}
        >
          {currentStep > DeploySteps.Prepared ? '✅ Encoded' : '2. Encode Resources'}
        </button>
        
        <button 
          onClick={() => handleWriteResources(5)}
          disabled={currentStep !== DeploySteps.Encoded || loading}
          className={currentStep > DeploySteps.Encoded ? 'completed' : ''}
        >
          {currentStep > DeploySteps.Encoded ? '✅ Written' : '3. Write Resources'}
        </button>
        ...
        >
          {currentStep > DeploySteps.Written ? '✅ Certified' : '4. Certify Resources'}
        </button>
        ...
        >
          {currentStep > DeploySteps.Certified ? '✅ Deployed' : '5. Write Site'}
        </button>
      </div>
