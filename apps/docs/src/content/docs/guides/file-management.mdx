...
await fileManager.removeFile('/old-file.txt');
...
function FileExplorer({ className }: FileExplorerProps) {
  ...
  const handleFileDelete = async (asset: IAsset) => {
    ...
    try {
      await fileManager.removeFile(asset.path);
...
Process files before deployment:

```typescript
import { getSHA256Hash, sha256ToU256 } from '@cmdoss/site-builder';

const processAssets = async (rawFiles: { path: string; content: Uint8Array }[]) => {
  const processedAssets = [];
  
  for (const file of rawFiles) {
    // Calculate hashes
    const hash = await getSHA256Hash(file.content);
    ...
    if (file.path.endsWith('.html') || file.path.endsWith('.css') || file.path.endsWith('.js')) {
      // Minify text files (implement your minification logic)
      processedContent = minifyContent(file.content, file.path);
    }
    ...
    processedAssets.push({
      ...
      processedSize: processedContent.byteLength
    });
  }
